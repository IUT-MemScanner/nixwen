...
bool Langue::load(string file) {
  ...
  while (!f.eof()) {
    ...
    if (str.empty() || str[0] == COMM_CHAR)
      continue;
    // spltit à tous les carractères '=' puis si des '=' se trouve dans la parti
    // definition (droite), les laisser
    vect = utils::explode(str, "=");
    if (vect.size() > 2) {
      std::string s;
      std::for_each(vect.begin() + 1, vect.end(),
    ...
    if (vect.size() == 2) {
      ...
      vect[1] = s;
      if (getString(vect[0], "") != "")
        clog << "Erase config" + vect[0] + " " + getString(vect[0], "") +
                    " by " + vect[1]
             << endl;
...
// liste des commandes
string Langue::quick_help() { return getString("quick_help", ""); }

string Langue::search_help() { return getString("search_help", ""); }
...
std::string Langue::welcome_msg() { return getString("welcome_msg", ""); }
